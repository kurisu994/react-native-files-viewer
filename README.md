...
# react-native-files-viewer [![npm version](https://img.shields.io/npm/v/react-native-files-viewer.svg?style=flat)](https://www.npmjs.com/package/react-native-doc-viewer)![Platform](https://img.shields.io/badge/platform-react--native%20%5Bios%20%26%20android%5D-blue.svg)![License](https://img.shields.io/npm/l/express.svg)

![nodei.co](https://nodei.co/npm/react-native-files-viewer.png?downloads=true&downloadRank=true&stars=true)
...
`$ react-native link react-native-files-viewer`
...
1. `npm install react-native-files-viewer --save`

2. Add the plugin dependency to your Podfile, pointing at the path where NPM installed it:

`pod 'RNFileViewer', path: '../node_modules/react-native-files-viewer'`
...
1. Open up `android/app/src/main/java/[...]/MainApplication.java`
  - Add `import com.kurisu.fileviewer.RNFileViewerPackage;` to the imports at the top of the file
  - Add `new RNFileViewerPackage()` to the list returned by the `getPackages()` method
2. Append the following lines to `android/settings.gradle`:
  	```
  	include ':react-native-files-viewer'
  	project(':react-native-files-viewer').projectDir = new File(rootProject.projectDir, 	'../node_modules/react-native-files-viewer/android')
  	```
3. Insert the following lines inside the dependencies block in `android/app/build.gradle`:
  	```
      implementation project(':react-native-files-viewer')
  	```
...
export default ({ navigation }: Props) => {
  ...
  function cb(error: any, fileName: string) {
    if (error) {
  ...
  function handlePress() {
    if (Platform.OS === 'android') {
      ...
      OpenFile.openDoc(
          {
            url: 'https://yoururl/public/demo.xlsx',
            fileName: 'demo1',
            cache: false,
            fileType: 'xlsx',
          },
   ...
   * select local file need use react-native-document-picker
   */
  async function handlePressLocal() {
    try {
